...
class Recommend(Singleton):
    ...
    #         partial_zincrby = partial(r.zincrby, name=f'product:{main_product_id}', amount=1)
    ...
    def suggest_items(self, product_id=None):
        # in product_detail
        if product_id:
            items = r.zrange(f'product:{product_id}', 0, -1, desc=True)[:6]
        # in product_list
        else:
            items = r.zrange('product', 0, -1, desc=True)[:6]
            print('items:', items)
